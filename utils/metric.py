...
class ConfusionMatrix:
    ...
    def compute(self):
        ...
        # NOTE: raise pytorch .any() error  
        # self.test_empty = not np.any(self.test)
        # self.test_full = np.all(self.test)
        # self.reference_empty = not np.any(self.reference)
        # self.reference_full = np.all(self.reference)

        test_np = np.asarray(self.test.detach().cpu() if hasattr(self.test, 'detach') else self.test)
        ref_np = np.asarray(self.reference.detach().cpu() if hasattr(self.reference, 'detach') else self.reference)
        self.test_empty = not np.any(test_np)
        self.test_full = np.all(test_np)
        self.reference_empty = not np.any(ref_np)
        self.reference_full = np.all(ref_np)
