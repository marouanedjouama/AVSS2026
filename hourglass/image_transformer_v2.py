...
class FourierFeatures(nn.Module):
    """Random Fourier feature embedding for scalar inputs."""
...
class NeighborhoodSelfAttentionBlock(nn.Module):
    ...
    def forward(self, x, pos, cond):
        ...
        # #TODO marouane change: throws an error 
        ...
        q_na = rearrange(q, "n nh h w e -> n h w nh e")
        k_na = rearrange(k, "n nh h w e -> n h w nh e")
        v_na = rearrange(v, "n nh h w e -> n h w nh e")

        flops.op(flops.op_natten, q_na.shape, k_na.shape, v_na.shape, self.kernel_size)

        x = natten.functional.na2d(q_na, k_na, v_na, self.kernel_size, scale=1.0)
